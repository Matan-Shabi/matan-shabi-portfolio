...
export default function ExperienceTimeline() {
  ...
  return (
    <div className="relative" ref={timelineRef}>
      ...
      <div
        ref={pipelineRef}
        className="relative h-1 w-full bg-gradient-to-r from-[#FF9900] to-[#FF5757] mb-12 rounded-full overflow-hidden"
      >
        {[...Array(10)].map((_, i) => (
          <div
            key={i}
            className="pipeline-dot absolute top-1/2 w-3 h-3 rounded-full bg-white transform -translate-y-1/2 opacity-0"
            style={{ left: 0 }}
          ></div>
        ))}
      </div>
      ...
      <div className="space-y-8 md:space-y-12">
        {experiences.map((exp, index) => (
          <div
            key={index}
            className="timeline-item opacity-0 transition-all duration-700 relative"
            style={{ transitionDelay: `${index * 200}ms` }}
          >
            <div className="bg-gradient-to-br from-[#232F3E] to-[#1A2433] rounded-lg p-6 border border-[#FF9900]/20 shadow-lg hover:shadow-[#FF9900]/10 transition-all duration-300 transform hover:scale-[1.02]">
            ...
            {index < experiences.length - 1 && (
              <div className="hidden md:block absolute left-1/2 bottom-0 h-8 w-0.5 bg-gradient-to-b from-[#FF9900] to-[#FF5757]/30 transform translate-x-[-50%] translate-y-[100%]"></div>
            )}
          </div>
        ))}
      </div>
